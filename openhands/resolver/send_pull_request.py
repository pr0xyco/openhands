...
def send_pull_request(
    ...
    base_branch_name = f'pr0xybot-fix-issue-{issue.number}'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'pr0xybot made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new pr0xybot update is available, but failed to parse or summarize the changes:\n{additional_message}'
